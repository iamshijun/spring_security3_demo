...
public class CustomJdbcDaoImpl extends JdbcDaoImpl implements IChangePassword {
	...
	/*
	 * UserDetailsManager 已经提供changPassword接口 这里可以使用JdbcUserDetailsManager!!(默认在声明了 <jdbc-user-service>时引入的 )
	 * 但可能需要在 js端传入md5编码过的 密文  或者是使用https后在controller层先md5 就像这里一样!
	 */
